/**
 * Function to list all designations from the Razorcopy API.
 *
 * @param {Object} args - Arguments for the request.
 * @param {number} [args.page=1] - The page number for pagination.
 * @param {number} [args.limit=10] - The number of items to return per page.
 * @param {string} [args.search] - The search term to filter designations.
 * @param {string} [args.sort] - The sorting order of the results.
...
const executeFunction = async ({ page = 1, limit = 10, search = '', sort = '' }) => {
  ...
  try {
    ...
    const url = new URL(`${host}/designations`);
...
/**
 * Tool configuration for listing designations from the Razorcopy API.
 * @type {Object}
 */
const apiTool = {
  ...
  definition: {
    ...
    function: {
      ...
      description: 'List all designations from the Razorcopy API.',
      parameters: {
        type: 'object',
        properties: {
          page: {
